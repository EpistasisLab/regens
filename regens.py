...
def main():
    ...
    if not os.path.isdir("input_files"):
        ...
        print(
            "Note: keep the input_files folder in your working directory if you intend to use them\n"
        )
    ...
    some_files_present = os.path.isdir("unit_testing_files") and os.path.isdir(
        "runtime_testing"
    )
    other_files_present = (
        os.path.isdir("examples")
        and os.path.isdir("correctness_testing_GBR")
        and os.path.isdir("correctness_testing_ACB")
    )
    all_files_present = some_files_present and other_files_present
    if not all_files_present:
        print(
            "Downloading examples and unit testing files to your working directory.\n"
        )
        ...
        os.remove(downloaded_path)
        print(
            "The examples and unit testing files have been downloaded to your working directory\n"
        )
    ...
    parser.add_argument(
        ...
        type=str,
        action="store",
        dest="input_plink_filename_prefix",
        default="empty",
        help="input plink filename prefix",
    )
    parser.add_argument(
        "--out",
        nargs="?",
        type=str,
        action="store",
        dest="output_plink_filename_prefix",
        default="simulated_whole_genomes",
        help="output plink filename prefix",
    )
    parser.add_argument(
        "--simulate_nbreakpoints",
        nargs="?",
        type=int,
        action="store",
        dest="num_breakpoints_simulated",
        default=4,
        help="If there are n breakpoints, then each simulated chromosome is comprised of n+1 chunks, where each chunk is from a randomly drawn input sample",
    )
    parser.add_argument(
        "--simulate_nsamples",
        nargs="?",
        type=int,
        action="store",
        dest="num_samples_simulated",
        default=1000,
        help="number of simulated",
    )
    parser.add_argument(
        "--phenotype", nargs="?", type=str, action="store", dest="phenotype", default=-9
    )
    parser.add_argument(
        "--mean_phenotype",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=str,
        action="store",
        dest="human_genome_version",
        default="empty",
    )
    parser.add_argument(
        "--recombination_file_path_prefix",
        nargs="?",
        type=str,
        action="store",
        dest="recombination_file_path_prefix",
        default="empty",
    )
    parser.add_argument(
        "--noise", nargs="?", type=float, action="store", dest="noise", default=0
    )
    parser.add_argument(
        "--population_ID",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=str,
        action="store",
        dest="major_minor_assignments_path",
        default="standard",
        help="input plink filename prefix",
    )
    parser.add_argument(
    ...
    if test_functionality == "test_units":
        if not (
            os.path.isfile("input_files/ACB.bed")
            and os.path.isfile("input_files/ACB.bim")
            and os.path.isfile("input_files/ACB.fam")
        ):
            print(
    ...
    if (
        os.path.isfile(input_plink_filename_prefix + ".bim")
        and os.path.isfile(input_plink_filename_prefix + ".bed")
        and os.path.isfile(input_plink_filename_prefix + ".fam")
    ):
        ...
        bim_SNP_pos_chr = [
            ((bim_file[bim_file[0] == chr])[3]).to_numpy() for chr in chromosomes
        ]
        bim_SNP_complete_pos_chr = [
            ((bim_file[bim_file[0] == chr])[[0, 2, 3]]).to_numpy()
            for chr in chromosomes
        ]
        bim_SNP_nucleotides_chr = [
            ((bim_file[bim_file[0] == chr])[[4, 5]]).to_numpy().astype("U")
            for chr in chromosomes
        ]
        bim_SNP_names = (bim_file[1]).to_numpy().astype("U")
        bim_SNP_names_chr = [
            ((bim_file[bim_file[0] == chr])[1]).to_numpy().astype("U")
            for chr in chromosomes
        ]
        ...
        print(
            "\nerror: one or more of the three necessary plink files with the filename prefix '"
    ...
    if num_breakpoints > 5:
        print(
            ...
            + str(num_breakpoints)
            + " breakpoints per chromosome, and there is no good reason to choose more than a few breakpoints per chromosome.\n"
        )
        print(
            "The simulation may become worse if there aren't roughly as many high recombination rate regions as there are breakpoints, and it may require large ammounts of time and memory.\n"
        )
    ...
    if phenotype == "binary":
        if mean_phenotype <= 0 or mean_phenotype >= 1:
            print(
                "The mean phenotype value for a binary phenotype must be greater than 0 ort less than 1"
            )
            exit()
    ...
    if (not (os.path.isdir("hg19") and os.path.isdir("hg38"))) and (
        hg_version == "hg19" or hg_version == "hg38"
    ):
        print(
            "Downloading recombination map files contained in hg19 and hg38 to your working directory. This may take a moment.\n"
        )
        print(
            "Note: When using regens' --human_genome_version flag, keep the hg19 and hg38 folders in your working directory.\n"
        )
    ...
    if rcmb_path_prefix != "empty":
        ...
        if pop_code in hg_pop_codes:
            ...
            rcmb_rate_info_path_prefix += (
                "_recombination_map_hapmap_format_" + hg_version + "_chr_"
            )
            try:
                rcmb_rate_info_file_names = np.sort(
                    os.listdir(hg_version + "/" + pop_code)
                )
                correct_info_file_names = np.sort(
                    [
                        pop_code
                        + "_recombination_map_hapmap_format_"
                        + hg_version
                        + "_chr_"
                        + str(i)
                        + ".txt.gz"
                ...
                print(
                    "error, possibility 2: the triadsim_main.py file needs to be in the same directory as the hg19 and hg38 folders.\n"
                )
                exit()
            try:
                ...
                print(
                    "\nerror: a recombination rate map file has been deleted or is incorrectly named in"
                    + rcmb_rate_info_path_prefix
                    + ".\n"
                )
                exit()
    ...
    print(
        ...
        + " samples and "
        + str(SNP_count)
        + " SNPs will require roughly "
        + minutes
        + " minutes and a maximum ram usage of "
        + max_ram
        + " GB.\n"
    )
    ...
    rcmb_rate_info_paths = [
        rcmb_rate_info_path_prefix + str(chr) + ".txt.gz" for chr in chromosomes
    ]
    rcmb_rate_info_chr = [
        pd.read_csv(path, delim_whitespace=True, header=0, compression="gzip")
        for path in rcmb_rate_info_paths
    ]
    if test_functionality == "test_units":
        rcmb_rate_info_chr = [
            reduce_recomb_rate_info(
                rcmb_rate_info_chr[i], bim_SNP_pos_chr[i], test_functionality
            )
            for i in [0]
        ]
    else:
    ...
    rcmb_rate_intervals_chr = [
        ((rcmb_rate_info_chr[i])["Position(bp)"]).to_numpy().reshape(-1)
        for i in range(len(rcmb_rate_info_chr))
    ]
    ...
    if noise < 0:
        print(
            "The ammount of noise as a percentage of the average signal cannot be negative."
        )
        exit()
    ...
    if not (
        os.path.isfile(causal_SNP_IDs_path)
        or (causal_SNP_IDs_path == "empty" and phenotype == -9)
    ):
        print(
            "\nerror: A causal SNP IDs file named '"
            + causal_SNP_IDs_path
            + "' does not exist in your working directory.\n"
        )
        exit()
    if not (os.path.isfile(betas_path) or (betas_path == "empty" and phenotype == -9)):
        print(
            "\nerror: A betas file named '"
            + betas_path
            + "' does not exist in your working directory.\n"
        )
        exit()
    if not (
        os.path.isfile(major_minor_assignments_path)
        or (major_minor_assignments_path == "standard")
    ):
        print(
            "\nerror: A major minor assignments file named '"
            + major_minor_assignments_path
            + "' does not exist in your working directory.\n"
        )
        exit()
    if not (
        os.path.isfile(SNP_phenotype_map_path) or (SNP_phenotype_map_path == "standard")
    ):
        print(
            "\nerror: A SNP phenotype map file named '"
            + SNP_phenotype_map_path
            + "' does not exist in your working directory.\n"
        )
        exit()
    ...
    for i in range(len(chromosomes)):
        ...
        input_sample_column_bounds = [
            cumulative_SNP_counts[i],
            cumulative_SNP_counts[i + 1],
        ]
        samples = get_samples_fast(
        ...
        breakpoints = draw_breakpoints(
            rcmb_rate_info_chr[i],
            bim_SNP_pos_chr[i],
            num_breakpoints,
            simulation_sample_size,
            test_functionality,
        ...
        simulated_individuals = breakpoints_to_simulated_individuals(
            breakpoints, chr_SNP_counts[i], samples, test_functionality
        )
    ...
    if (
        phenotype == "binary" or phenotype == "continuous"
    ) and test_functionality != "test_units":
        ...
        simulated_phenotypes = simulate_phenotypes(
            output_file_names,
            causal_SNP_IDs_path,
        ...
        if phenotype == "binary":
            legend_label = "phenotype barplot (mean = " + str(
                np.round(np.mean(simulated_phenotypes), 5)
            )
            legend_label += (
                ", min = "
                + str(np.round(np.min(simulated_phenotypes), 5))
                + ", max = "
                + str(np.round(np.max(simulated_phenotypes), 5))
                + ")"
            )
            plt.bar(
                [0, 1],
                [np.sum(simulated_phenotypes == 0), np.sum(simulated_phenotypes == 1)],
                label=legend_label,
                tick_label=["0", "1"],
            )
        if phenotype == "continuous":
            legend_label = "phenotype value histogram (mean = " + str(
                np.round(np.mean(simulated_phenotypes), 5)
            )
            legend_label += (
                ", min = "
